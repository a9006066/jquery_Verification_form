...
function getAdbpm (){
  ...
  if(parAddPol==='1' || parAddPol==='2'){
    ...
    if(parAddPol==='1' && parMoney*0.05*10%10 === 5){
     ...
     adbpmTemp.forEach((_item)=> {adbpmSUM += _item.BPM;});
  ...
  //正常狀態
     ...
     var v = parAddPol === '1'? 0.05 : 0.1;
      ...
      var adbpmTemp = datas.filter((_item)=>{
        return _item.TADAY.toString() === parDays && (((parPG === 'AE'|| parPG === 'AF') && _item.TACODE ==='MS')
        || ((parPG === 'AJ'|| parPG === 'AK') && _item.TACODE ==='MF')) && _item.BSF === parMoney*v ;
      }) ;
  ...
  //--傷害醫療+海外疾病住院醫療(3=5%,4=10%) start
   ...
   if(parAddPol==='3' && parMoney*0.05*10%10 === 5){
    ...
    var adbpm2TempA = datas.filter((_item)=>{
      return _item.TADAY.toString() === parDays && (parPG === 'SM' && _item.BSF === parMoney*0.05-0.5) 
    })[0].BPM ; 
   ...
   //--正常狀態
    ...
    var adbpmTemp = datas.filter((_item)=>{
      return _item.TADAY.toString() === parDays && (((parPG === 'AE' || parPG === 'AF') && (_item.TACODE ==='MS' || _item.TACODE ==='SM'))
